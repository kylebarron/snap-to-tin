...
export function snapFeatures(options = {}) {
  ...
  for (const feature of features) {
    if (getType(feature) === "Point") {
      ...
      // Clip to box
      const clippedFeature = bboxClip(feature, bounds);
      ...
      clippedFeature.geometry.coordinates = handleLineString(coords, tree)
...
function handleLineString(line, tree) {
  ...
  for (let i = 0; i < line.length - 1; i++) {
    ...
    const newStart = handlePoint(start, tree);
