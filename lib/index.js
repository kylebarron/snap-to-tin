...
function handlePoint(point, tree) {
    ...
    var [ax, ay, az] = filteredResult.feature.geometry.coordinates[0][0]
    var [bx, by, bz] = filteredResult.feature.geometry.coordinates[0][1]
    var [cx, cy, cz] = filteredResult.feature.geometry.coordinates[0][2]

    // Find the mix of a, b, and c to use
    var mix = barycentric([[ax, ay], [bx, by], [cx, cy]], point)
    // Find the correct z based on that mix
    var interpolatedZ = (mix[0] * az) + (mix[1] * bz) + (mix[2] + cz)
    return [...point, interpolatedZ];
}
