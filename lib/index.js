...
export function snapFeaturesToTerrain({ terrain, features }) {
  ...
  for (var feature of features) {
    if (getType(feature) === "Point") {
      ...
      var point = handlePoint(coord, tree);
      point.properties = feature.properties;
      newFeatures.push(point)
...
function handlePoint(point, tree) {
  ...
  // filteredResults[0].feature.geometry.coordinates
  // console.log(JSON.stringify(Feature(filteredResults[0].feature)))
  // filteredResults[0].feature.geometry.coordinates
  // console.log(JSON.stringify(filteredResults[0].feature))
  // console.log(JSON.stringify(filteredResults[1].feature))
  //
  // filteredResults[0].feature.geometry.coordinates[0] = filteredResults[0].feature.geometry.coordinates[0].map(x => Object.values(x))
  // filteredResults[1].feature.geometry.coordinates[0] = filteredResults[1].feature.geometry.coordinates[0].map(x => Object.values(x))
  // console.log(JSON.stringify(Point(point)))

  // test = filteredResults.map(result => {
